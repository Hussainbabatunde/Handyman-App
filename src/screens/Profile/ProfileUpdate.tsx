...
export default function ProfileUpdateForm({ user, onSubmit }: any) {
    ...
    const {uploadedImage, UploadDocmentApi, isSubmitting, updateProfileApiCall, fileSentName} = useContext<any>(ProfileContext)
  ...
  const [profileImg, setProfileImg] = useState(userData?.user?.profileImg || "");
//   console.log("userData: ", userData);
  ...
  useEffect(()=>{
    if(uploadedImage){
      if(fileSentName == "ProfileImg"){
        setProfileImg(uploadedImage);
      }
      else{
...
const handleSubmit = async (values: any) => {
    // console.log("works");
    let sentData = {...values, previousWork, profileImg}
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "white" }}>
      <ScrollView style={{ padding: 20 }}>
        ...
        <Pressable onPress={selectProfilePic} style={styles.profileWrapper}>
          {profileImg ? (
