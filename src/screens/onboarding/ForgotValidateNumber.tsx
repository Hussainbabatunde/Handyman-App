...
export default function ForgotValidatePhone () {
  ...
  const {verifyPhoneRes, phoneNumber, forgotValidatePhoneApiCall, isSubmitting, resendPhoneOtpApiCall} = useContext(OnboardContext)
  const route = useRoute<any>()
//   const { phoneNo } = route.params;
const {identifierName} = useContext<any>(AppContext)
...
useEffect(() => {
    ...
    useEffect(()=>{
        ...
        if(code?.length == MAX_CODE_LENGTH){
            ...
            let sentData = {
                otp: code,
                phoneNo: phoneNumber || identifierName,
                sessionId: verifyPhoneRes?.sessionId
            }
            await forgotValidatePhoneApiCall(sentData)
        }
    }
    initiate()
    },[code])
        ...
        <SafeAreaView style={styles.container}>
            <ScrollView style={styles.BodySpacing}>
             <View style={{ flexDirection: "row", alignItems: "center", marginTop: StatusBar.currentHeight }}>
                    ...
                    <Text style={styles.descText}>Enter the 4-digit code sent to you at <Text style={[styles.descText, {fontWeight: 600}]}>{maskMiddle(identifierName || phoneNumber)}</Text></Text>
