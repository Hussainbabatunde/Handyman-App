...
import { completeBookingApi, getAllArtisanBookingsApi, getAllBookingsApi, getBookingDetailsApi, updateBookingDetailsApi } from '../../services';
...
const BookingsNavigation = ({navigation, route}: any) => {
  ...
  const [updateBookingDetailsRes, setupdateBookingDetailsRes] = useState<any>(null)
...
const getAllBookingsApiCall = async (key: string) => {
    ...
    let userToken = await AsyncStorage.getItem("userToken");
      await getAllArtisanBookingsApi(userToken)
    ...
    const updateBookingDetailsApiCall = async (id: string, values: object) => {
      Keyboard.dismiss();
      
      setIsSubmitting((prev) => ({...prev, bookingDetails: true}));
      
    let userToken = await AsyncStorage.getItem("userToken");
      await updateBookingDetailsApi(userToken, id, values)
        .then((response) => response)
        .then(async (data) => {
          setupdateBookingDetailsRes(data)
      setIsSubmitting((prev) => ({...prev, bookingDetails: false}));
        })
        .catch((error) => {
      setIsSubmitting((prev) => ({...prev, bookingDetails: false}));
  ...
  return (
    <BookingsContext.Provider
      value={{
        ...
        updateBookingDetailsRes, 
        setupdateBookingDetailsRes,
        updateBookingDetailsApiCall
      }}>
